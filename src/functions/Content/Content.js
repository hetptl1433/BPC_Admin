...
export const createContent = async (values) => {
  return await axios.post(
    `${process.env.REACT_APP_API_URL_BPC}/api/auth/Content`,
    values,
    {
        headers: {
...
export const removeContent = async (_id) => {
  return await axios.delete(
    `${process.env.REACT_APP_API_URL_BPC}/api/auth/remove/Content/${_id}`
  );
};
...
export const updateContent = async (_id, values) => {
  return await axios.put(
    `${process.env.REACT_APP_API_URL_BPC}/api/auth/update/Content/${_id}`,
    values,
    {
        headers: {
          'Content-Type': 'multipart/form-data'
...
export const getContent = async (_id) => {
  return await axios.get(
    `${process.env.REACT_APP_API_URL_BPC}/api/auth/get/Content/${_id}`
  );
};
