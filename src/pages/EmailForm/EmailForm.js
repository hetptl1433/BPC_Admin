...
import { listEmailMaster } from "../../functions/EmailMaster/EmailMaster";
import { createEmailFormDetails, getEmailFormDetails, removeEmailFormDetails, updateEmailFormDetails } from "../../functions/EmailFormFun/EmailFormFun";
...
const EmailForm = () => {
  ...
  const fetchProducts = async () => {
    ...
    await axios
      .post(
        `${process.env.REACT_APP_API_URL_BPC}/api/auth/list-by-params/EmailForm-details`,
        {
          skip: skip,
          per_page: perPage,
          sorton: column,
  ...
  const loadEmailss = () => {
    listEmailMaster().then((res) => setLEDCategories(res));
  };
  ...
  const handleClick = (e) => {
    ...
    if (Object.keys(errors).length === 0) {
      ...
      createEmailFormDetails(formdata)
        .then((res) => {
  ...
  const handleDelete = (e) => {
    e.preventDefault();
    removeEmailFormDetails(remove_id)
      .then((res) => {
        setmodal_delete(!modal_delete);
        fetchProducts();
  ...
  const handleUpdate = (e) => {
    ...
    const formdata = new FormData();
      ...
      formdata.append("FormName", values.FormName);
      formdata.append("IsActive", values.IsActive);
    updateEmailFormDetails(_id, formdata)
      .then((res) => {
