...
const ServeFiles = () => {
  ...
  const { Title, ServeFile,SortOrder, IsActive } = values;
 ...
 if (values.SortOrder !== "") {
   setErrTT(false);
    ...
    if (values.ServeFile === "") {
      errors.ServeFile = "Serve File is required!";
      setErrBI(true);
    }
    if (values.ServeFile !== "") {
      setErrBI(false);
  ...
  const handleClick = (e) => {
    ...
    if (Object.keys(erros).length === 0) {
      ...
      formdata.append("myFile", values.ServeFile);
     
      formdata.append("IsActive", values.IsActive);
      formdata.append("Title", values.Title);
      formdata.append("SortOrder", values.SortOrder);

      createServeFiles(formdata)
        .then((res) => {
          setmodal_list(!modal_list);
          setValues(initialState);
...
toast.success("Data submitted successfully");
  ...
  const fetchCategories = async () => {
    ...
    await axios
      .post(
        `${process.env.REACT_APP_API_URL_BPC}/api/auth/list-by-params/ServeFiles`,
        {
          skip: skip,
          per_page: perPage,
          sorton: column,
  ...
  const PhotoUpload = (e) => {
    if (e.target.files.length > 0) {
      ...
      setValues({ ...values, ServeFile: e.target.files[0] });
  ...
  const col = [
    ...
    {
      name: "Image",
      selector: (row) => renderImage(row.ServeFile),
      sortable: false,
      sortField: "Image",
      minwidth: "150px",
    },
      ...
      >
        ...
        <form>
          <ModalBody>
            ...
            <Col lg={6}>
              <label>
                Serve Image <span className="text-danger">*</span>
              </label>
              ...
              />
              {isSubmit && (
                <p className="text-danger">{formErrors.ServeFile}</p>
              )}
              {checkImagePhoto ? (
                <img
                  //   src={image ?? myImage}
              ...
              {values.ServeFile || photoAdd ? (
                <img
                  ...
                  src={
                    checkImagePhoto
                      ? photoAdd
                      : `${process.env.REACT_APP_API_URL_BPC}/${values.ServeFile}`
                  }
                  width="300"
                  height="200"
                />
