...
import { createEmailControl, getEmailControl, removeEmailControl, updateEmailControl } from "../../functions/EmailFun/EmailFun";
...
const [IsActive, setIsActive] = useState(false);
  ...
  const handleUpdate = (e) => {
    e.preventDefault();
    let erros = validate(Title, Email);
    setFormErrors(erros);
    setIsSubmit(true);
    ...
    if (Object.keys(erros).length === 0) {
        ...
        formdata.append("Email", Email);
        formdata.append("IsActive", IsActive);
      updateEmailControl(_id, formdata)
        .then((res) => {
          setmodal_edit(!modal_edit);
          fetchCategories();
        })
