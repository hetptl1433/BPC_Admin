...
const MediaFiles = () => {
  ...
  const { Title, MediaFile, IsActive } = values;
  ...
  const validate = (values) => {
    ...
    if (values.MediaFile === "") {
      errors.MediaFile = "Media Image is required!";
      setErrBI(true);
    }
    if (values.MediaFile !== "") {
      setErrBI(false);
  ...
  const handleClick = (e) => {
    ...
    if (Object.keys(erros).length === 0) {
      ...
      formdata.append("myFile", values.MediaFile);
     
      formdata.append("IsActive", values.IsActive);
      formdata.append("Title", values.Title);

      createMediaFiles(formdata)
        .then((res) => {
          setmodal_list(!modal_list);
          setValues(initialState);
  ...
  const fetchCategories = async () => {
    ...
    await axios
      .post(
        `${process.env.REACT_APP_API_URL_BPC}/api/auth/list-by-params/MediaFiles`,
        {
          skip: skip,
          per_page: perPage,
          sorton: column,
  ...
  const PhotoUpload = (e) => {
    if (e.target.files.length > 0) {
      ...
      setValues({ ...values, MediaFile: e.target.files[0] });
  ...
  const col = [
    ...
    {
      name:"Image",
      selector: (row) => renderImage(row.MediaFile),
      sortable: false,
      sortField: "Image",
      minwidth: "150px"
    },
                ...
                <CardHeader>
                  <Row className="g-4 mb-1">
                    <Col className="col-sm" sm={6} lg={4} md={6}>
                      <h2 className="card-title mb-0 fs-4 mt-2">MediaFiles</h2>
                    </Col>
        ...
        <form>
          <ModalBody>
            ...
            <Col lg={6}>
              ...
              />
              {isSubmit && (
                <p className="text-danger">{formErrors.MediaFile}</p>
              )}
              {checkImagePhoto ? (
                <img
                  //   src={image ?? myImage}
              ...
              {values.MediaFile || photoAdd ? (
                <img
                  ...
                  src={
                    checkImagePhoto
                      ? photoAdd
                      : `${process.env.REACT_APP_API_URL_BPC}/${values.MediaFile}`
                  }
                  width="300"
                  height="200"
                />
