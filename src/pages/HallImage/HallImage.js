...
const HalleBoard = () => {
  ...
  const fetchProducts = async () => {
    ...
    await axios
      .post(
        `${process.env.REACT_APP_API_URL_COFFEE}/api/auth/list-by-params/Halleboard-details`,
        {
          skip: skip,
          per_page: perPage,
          sorton: column,
  ...
  const [HalleCategories, setHalleCategories] = useState([]);
  ...
  const handleClick = (e) => {
    ...
    if (Object.keys(errors).length === 0) {
      ...
      createHalleBoardDetails(formdata)
        .then((res) => {
  ...
  const handHalleelete = (e) => {
    e.preventDefault();
    removeHalleBoardDetails(remove_id)
      .then((res) => {
        setmodal_delete(!modal_delete);
        fetchProducts();
      })
  ...
  const handleUpdate = (e) => {
    ...
    formdata.append("myFile", values.productImage);
      ...
      formdata.append("SortOrder", values.SortOrder);
      formdata.append("IsActive", values.IsActive);
    updateHalleBoardDetails(_id, formdata)
      .then((res) => {
