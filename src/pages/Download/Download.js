...
import {
  ...
  removeDownloadFiles,
  updateDownloadFiles,
} from "../../functions/Download/Download";
const initialState = {
  Title: "",
...
const DownloadFiles = () => {
  ...
  const { Title, DownloadFile,SortOrder, IsActive } = values;
  ...
  const handleClick = (e) => {
    ...
    if (Object.keys(erros).length === 0) {
      ...
      formdata.append("myFile", values.DownloadFile);
     
      formdata.append("IsActive", values.IsActive);
      formdata.append("Title", values.Title);
      formdata.append("SortOrder", values.SortOrder);

      createDownloadFiles(formdata)
        .then((res) => {
          setmodal_list(!modal_list);
          setValues(initialState);
  ...
  const handleUpdate = (e) => {
    ...
    if (Object.keys(erros).length === 0) {
      ...
      updateDownloadFiles(_id, formdata)
        .then((res) => {
          setmodal_edit(!modal_edit);
          fetchCategories();
          setPhotoAdd("");
  ...
  const fetchCategories = async () => {
    ...
    await axios
      .post(
        `${process.env.REACT_APP_API_URL_BPC}/api/auth/list-by-params/DownloadFiles`,
        {
          skip: skip,
          per_page: perPage,
          sorton: column,
  ...
  const col = [
    ...
    {
      name: "Image",
      selector: (row) => renderImage(row.DownloadFile),
      sortable: false,
      sortField: "Image",
      minwidth: "150px",
    },
        ...
        <form>
          <ModalBody>
            ...
            <Col lg={6}>
              ...
              />
              {isSubmit && (
                <p className="text-danger">{formErrors.DownloadFile}</p>
              )}
              {checkImagePhoto ? (
                <img
                  //   src={image ?? myImage}
              ...
              {values.DownloadFile || photoAdd ? (
                <img
                  ...
                  src={
                    checkImagePhoto
                      ? photoAdd
                      : `${process.env.REACT_APP_API_URL_BPC}/${values.DownloadFile}`
                  }
                  width="300"
                  height="200"
                />
